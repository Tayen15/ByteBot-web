...
export default function OwnerBotSettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setAlert({ message: 'Bot settings updated successfully', type: 'success' });
      } else {
      ...
      <div className="mb-10">
        <div className="flex items-center gap-4 mb-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-bold text-white">Global Bot Settings</h1>
          </div>
        </div>
        <p className="text-text-secondary ml-14">Configure global bot settings including the core AI Model.</p>
      </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="bg-dark-card border border-border-dark rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-semibold mb-2 text-white">AI Core Model</label>
                <div className="relative">
                  <select 
                    value={settings.aiModel}
                    onChange={(e) => handleChange('aiModel', e.target.value)}
                    className="w-full bg-dark-secondary border border-border-dark rounded-lg px-4 py-3 text-white focus:outline-none focus:border-discord transition-colors appearance-none"
                  >
                    <option value="glm-5.2-free">GLM 5.2 Free (Default)</option>
                ...
                <p className="text-xs text-text-secondary mt-2">Select the underlying AI model used for all Discord Chatbot interactions globally. Ensure your API Key supports the selected model.</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-dark-card border border-border-dark rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-semibold mb-2 text-white">Activity Type</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-semibold mb-2 text-white">Activity Text</label>
                <input 
                  type="text"
